...
const sampleTimetable = {
    "Monday": [
        { time: "08:00-10:00", subject: "Programming 1", venue: "IT Building 101", lecturer: "Dr. Smith", isOnline: false },
        { time: "10:00-12:00", subject: "Mathematics 1", venue: "Science Building 201", lecturer: "Prof. Johnson", isOnline: false, venueChanged: true, oldVenue: "Science Building 101" }
    ],
    "Tuesday": [
        { time: "09:00-11:00", subject: "Database Systems", venue: "Online - MS Teams", lecturer: "Ms. Williams", isOnline: true }
    ],
    "Wednesday": [
        { time: "08:00-12:00", subject: "Web Development", venue: "IT Lab 3", lecturer: "Mr. Brown", isOnline: false }
    ],
    "Thursday": [
        { time: "10:00-12:00", subject: "Networking", venue: "IT Building 103", lecturer: "Dr. Davis", isOnline: false }
    ],
    "Friday": [
        { time: "14:00-16:00", subject: "Project Session", venue: "Project Lab 1", lecturer: "All Lecturers", isOnline: false }
    ]
};
...
const greetings = [
    "Welcome to our TUT!",
    "Wamkelekile kwi-TUT yethu!",
    "Welkom by ons TUT!",
    "Siyakwamukela e-TUT yethu!",
    "Re a go amogela ho TUT ya rona!",
    "Re a le amogela kwa TUT ya rona!",
    "Dumela go TUT ya rona!",
    "Mi avuxeriwe eka TUT ya hina!",
    "Ndi a ni ṱanganedza kha TUT yashu!",
    "Siyakwemukela e-TUT yetfu!",
    "Siyakwamukela e-TUT yethu!",
    "Bienvenue à notre TUT!"
];
...
class CampusPalApp {
    ...
    getHomePage() {
        return `
            <div class="home-container" style="background-image: url('images/grooupphoto.jpeg');">
                <div class="welcome-message">
                    <h1>Campus Pal</h1>
                    <div class="language-greeting">
                        ${greetings.map((greeting, index) => 
                            `<div class="greeting-slide" style="animation-delay: ${index * 3}s">
                                ${greeting}
                            </div>`
                        ).join('')}
                    </div>
    ...
    getTimetablePage() {
        ...
        return `
            <div class="timetable-container">
                ...
                <div class="timetable-view" id="timetableView" data-current-view="week" data-current-day="${today}">
                    ${this.renderTimetableView('week', today)}
                </div>
            </div>
    ...
    renderTimetableView(view, day) {
        if (view === 'day') {
            const classes = sampleTimetable[day] || [];
            if (classes.length === 0) {
                return `<div class="no-classes"><i class="fas fa-calendar-times"></i><p>No classes scheduled for ${day}</p></div>`;
            }
            return `
    ...
    renderClassCard(cls) {
        const onlineBadge = cls.isOnline ? '<span class="online-badge"><i class="fas fa-video"></i> Online Class</span>' : '';
        const venueAlert = cls.venueChanged ? `<div class="venue-alert"><i class="fas fa-exclamation-triangle"></i> Venue Changed from ${cls.oldVenue}</div>` : '';
        const navigateBtn = !cls.isOnline ? `<button class="btn-navigate"><i class="fas fa-directions"></i> Navigate to Class</button>` : `<button class="btn-join-online"><i class="fas fa-video"></i> Join Online</button>`;
        
        return `
            <div class="class-card ${cls.isOnline ? 'online-class' : ''}" data-venue="${cls.venue}">
                ${onlineBadge}
                ${venueAlert}
    ...
    setupTimetableEvents() {
        ...
        // Join online buttons
        document.querySelectorAll('.btn-join-online').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                alert('Opening MS Teams...\n\nIn production, this would launch the online meeting link.');
            });
        });
        ...
        document.querySelectorAll('.btn-view').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                const currentDay = timetableView.dataset.currentDay;
                ...
                // Update view
                timetableView.dataset.currentView = view;
                timetableView.innerHTML = this.renderTimetableView(view, currentDay);
    ...
    showNavigation(venue) {
        const steps = [
            "Start at the small gate entrance to campus",
            "Walk straight along the main pathway",
            "Pass by the administration building on your left",
            `Continue towards ${venue}`,
        ...
        document.getElementById('navigationSteps').innerHTML = `
            <div class="navigation-header">
                ...
                <p>Follow the route from the small gate entrance</p>
            </div>
            
            <div class="campus-map">
                <div class="map-marker start-marker" title="Small Gate Entrance"></div>
                ...
                <div style="position: absolute; top: 10px; left: 40px; font-size: 12px; color: var(--tut-blue); font-weight: bold;">Small Gate</div>
                ...
                <form class="registration-form" id="studentCardForm">
                    ...
                    <div class="input-group">
                        ...
                        <select id="campus" required>
                            ...
                            <option value="garankuwa">Garankuwa Campus</option>
                    ...
                    <div class="input-group">
                        ...
                        <select id="studentType" required>
                            ...
                            <option value="lost">Lost Card Replacement</option>
                        </select>
                    </div>

                    <div id="lostCardSection" style="display: none;">
                        <div class="input-group">
                            <label>Proof of Payment (R100 Replacement Fee)</label>
                            <input type="file" id="proofPayment" accept=".pdf,.jpg,.png">
                        </div>
                    </div>

                    <div id="returningStudentSection" style="display: none;">
                        <div class="input-group">
                            <label>Photo of Current Student Card (Proof you still have it)</label>
                            <input type="file" id="currentCardPhoto" accept=".jpg,.png">
                            <small style="color: var(--text-gray);">Take a clear photo of your current student card - No payment required</small>
                        </div>
                    </div>
                    ...
                    <div class="upload-section" id="signatureUpload">
                        <i class="fas fa-signature fa-3x"></i>
                        <h4>Signature</h4>
                        <p>Upload or capture your signature</p>
                        <div class="upload-options">
                            <button type="button" class="btn-upload" id="uploadSignature">
                    ...
                    <div id="busSticker" style="display: none;">
                        <div class="input-group">
                            <label>Do you want a bus sticker?</label>
                            <select id="busOption">
                    ...
                    <div class="input-group">
                        ...
                        <select id="deliveryMethod" required>
                            ...
                            <option value="residence">Deliver to Residence</option>
                        </select>
                    </div>

                    <div id="residenceAddress" style="display: none;">
                        <div class="input-group">
                            <label>Residence Name/Address</label>
                            <textarea id="residenceDetails" rows="3" placeholder="Enter your residence name and room number" style="width: 100%; padding: 15px; border: 2px solid #ddd; border-radius: 10px; font-size: 1rem;"></textarea>
                        </div>
                    </div>
    ...
    setupStudentCardEvents() {
        ...
        document.getElementById('studentType')?.addEventListener('change', (e) => {
            const lostCardSection = document.getElementById('lostCardSection');
            const returningStudentSection = document.getElementById('returningStudentSection');
            
            if (e.target.value === 'lost') {
                lostCardSection.style.display = 'block';
                returningStudentSection.style.display = 'none';
            } else if (e.target.value === 'returning') {
                lostCardSection.style.display = 'none';
                returningStudentSection.style.display = 'block';
            } else {
                lostCardSection.style.display = 'none';
                returningStudentSection.style.display = 'none';
            }
        ...
        document.getElementById('residenceType')?.addEventListener('change', (e) => {
            const busSticker = document.getElementById('busSticker');
            if (e.target.value === 'on-campus') {
                busSticker.style.display = 'none';
                // On-campus students automatically get bus sticker
            } else if (e.target.value === 'off-campus') {
                busSticker.style.display = 'block';
            } else {
                busSticker.style.display = 'none';
            }
        });
    ...
    submitStudentCardApplication() {
        ...
        const busSticker = residenceType === 'on-campus' ? 'Yes (Automatic)' : document.getElementById('busOption')?.value || 'N/A';
        const residenceDetails = deliveryMethod === 'residence' ? document.getElementById('residenceDetails').value : 'N/A';
        
        const campusNames = {
            'arcadia': 'Arcadia Campus',
            'main': 'Main Campus',
            'soshanguve-south': 'Soshanguve South Campus',
            'soshanguve-north': 'Soshanguve North Campus',
            'garankuwa': 'Garankuwa Campus',
            'arts': 'Arts Campus',
        ...
        document.getElementById('statusContent').innerHTML = `
            ...
            <div class="status-details">
                ...
                <p><strong>Campus:</strong> ${campusNames[campus]}</p>
                <p><strong>Student Type:</strong> ${studentType.charAt(0).toUpperCase() + studentType.slice(1)}</p>
                <p><strong>Residence:</strong> ${residenceType === 'on-campus' ? 'On Campus' : 'Off Campus'}</p>
                <p><strong>Bus Sticker:</strong> ${busSticker}</p>
                <p><strong>Delivery Method:</strong> ${deliveryMethod === 'collect' ? 'Collect at Campus' : 'Deliver to Residence'}</p>
                ${deliveryMethod === 'residence' ? `<p><strong>Delivery Address:</strong> ${residenceDetails}</p>` : ''}
                ...
                <p><strong>Collection Point:</strong> ${deliveryMethod === 'collect' ? 'Student Administration Building, Room 101' : 'Your Residence'}</p>
            ...
            <div class="map-container">
                ...
                <div class="map-display">
                    <div class="campus-map-view">
                        <img src="images/campus_map.jpeg" alt="Soshanguve South Campus Map" class="campus-map-image">
        ...
        return `
            <div class="profile-container">
                <div class="profile-header">
                    <div class="profile-avatar" id="profileAvatar" style="${profilePic ? `background-image: url(${profilePic}); background-size: cover; background-position: center;` : ''}">
                        ${profilePic ? '' : this.currentUser?.name.charAt(0)}
